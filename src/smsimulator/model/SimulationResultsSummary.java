...
public class SimulationResultsSummary {
	...
	// The index 0 of this ArrayList contains the Total Capital for the Company with
	// ID 1 and so on
	public ArrayList<Double> getCompaniesCapital(ArrayList<Transaction> transactions, ArrayList<Company> companies) {
		...
		for (Transaction tran : transactions) {
			companiesCapital.set(tran.getCompany().getId(),
					companiesCapital.get(tran.getCompany().getId()) + tran.getTrasSharePrice());
	...
	public ArrayList<Integer> getInvestorsNumberOfShares(ArrayList<Transaction> transactions,
		...
		for (Transaction tran : transactions) {
			investorsNumberOfShares.set(tran.getInvestor().getId(),
					investorsNumberOfShares.get(tran.getInvestor().getId()) + 1);
		}
	...
	public ArrayList<Integer> getNumberOfCompaniesByInvestor(ArrayList<Transaction> transactions,
			ArrayList<Investor> investors) {
		// List to keep track of the IDs of investor and companies for every
		// transaction
		...
		for (Transaction tran : transactions) {
			...
			if (newCompany == true) {
				...
				numberOfCompaniesByInvestor.set(tran.getInvestor().getId(),
						numberOfCompaniesByInvestor.get(tran.getInvestor().getId()) + 1);
			}
		}
