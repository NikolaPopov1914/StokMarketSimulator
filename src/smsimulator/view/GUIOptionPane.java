...
public class GUIOptionPane implements Component {
	...
	public void inputDialog() {
		...
		int result = JOptionPane.showConfirmDialog(null, myPanel, "Enter the number of Companies and Investors",
		...
		if (result == JOptionPane.OK_OPTION) {
			if (!numberCompanies.getText().equals("") && !numberInvestors.getText().equals("")
				...
				if (numberCompaniesInt > 0 && numberCompaniesInt <= 100 && numberInvestorsInt > 0
						&& numberInvestorsInt <= 100) {

					mediator.setNumberCompanies(numberCompaniesInt);
					mediator.setNumberInvestors(numberInvestorsInt);
					mediator.setNewSim(true);
				} else {
					JOptionPane.showMessageDialog(null, "Please enter an integer between 1 and 100 (No blank space)",
